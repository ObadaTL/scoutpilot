"""ScoutPilot configuration: paths, platform detection, user data."""
...
def load_profile() -> dict:
    ...
    if not PROFILE_PATH.exists():
        raise FileNotFoundError(
            f"Profile not found at {PROFILE_PATH}. Run `scoutpilot init` first."
        )
...
DEFAULTS = {
    ...
    # separate `scoutpilot run tailor`/`run cover` invocations a job gets
...
def check_tier(required: int, feature: str) -> None:
    ...
    if required >= 2 and not any(os.environ.get(k) for k in ("GEMINI_API_KEY", "OPENAI_API_KEY", "LLM_URL")):
        missing.append("LLM API key — run [bold]scoutpilot init[/bold] or set GEMINI_API_KEY")
    if required >= 3:
