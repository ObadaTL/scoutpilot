...
from scoutpilot.config import get_currency_symbol, get_locale_style
...
def _build_profile_summary(profile: dict) -> str:
    ...
    lines.append(f"Salary Expectation: {symbol}{comp['salary_expectation']} {currency}")
...
def _build_salary_section(profile: dict) -> str:
    ...
    Never assumes USD -- the symbol and every example follow salary_currency,
    and the "senior role" floor is derived from the candidate's own stated
    range rather than a hardcoded US-market dollar figure.
    """
    ...
    # Compute example hourly rates spanning the candidate's own range
    try:
        floor_int = int(floor)
        range_max_int = int(range_max)
        mid_int = (floor_int + range_max_int) // 2
        examples = [
            (f"{prefix}{floor_int // 1000}K", floor_int // 2080),
            (f"{prefix}{mid_int // 1000}K", mid_int // 2080),
            (f"{prefix}{range_max_int // 1000}K", range_max_int // 2080),
        ]
        hourly_line = ", ".join(f"{sal} = {prefix}{hr}/hr" for sal, hr in examples)
        senior_floor = f"{prefix}{range_max}"
...
{prefix}{floor} {currency} is the FLOOR. Never go below it. But don't always use it either.

Decision tree:
1. Job posting shows a range? -> Answer with the MIDPOINT.
2. Title says Senior, Staff, Lead, Principal, Architect, or level II/III/IV? -> Minimum {senior_floor} {currency} (the top of your stated range). Use midpoint of posted range if higher.
3. {convert_line}
4. No salary info anywhere? -> Use {prefix}{floor} {currency}.
5. Asked for a range? -> Give posted midpoint minus 10% to midpoint plus 10%. No posted range? -> "{prefix}{range_min}-{prefix}{range_max} {currency}".
...
def build_prompt(job: dict, tailored_resume: str,
    ...
    doc_word = "CV" if get_locale_style(profile)["doc_name"] == "CV" else "Resume"
    dest_dir = config.APPLY_WORKER_DIR / "current"
    dest_dir.mkdir(parents=True, exist_ok=True)
    upload_pdf = dest_dir / f"{name_slug}_{doc_word}.pdf"
    ...
    from scoutpilot.config import load_blocked_sso
