"""ScoutPilot Pipeline Orchestrator.

Runs pipeline stages in sequence or concurrently (streaming mode).

Usage (via CLI):
    scoutpilot run                        # all stages, sequential
    scoutpilot run --stream               # all stages, concurrent
    scoutpilot run discover enrich        # specific stages
    scoutpilot run score tailor cover     # LLM-only stages
...
from scoutpilot.database import init_db, get_connection, get_stats, start_run, end_run
...
def _run_enrich(workers: int = 1) -> dict:
    ...
    try:
        from scoutpilot.enrichment.detail import run_enrichment
...
def _run_tailor(min_score: int = 7, validation_mode: str = "normal") -> dict:
    ...
    try:
        from scoutpilot.scoring.tailor import run_tailoring
...
def _run_cover(min_score: int = 7, validation_mode: str = "normal") -> dict:
    ...
    try:
        from scoutpilot.scoring.cover_letter import run_cover_letters
...
def _run_pdf() -> dict:
    ...
    try:
        from scoutpilot.scoring.pdf import batch_convert
        batch_convert()
        return {"status": "ok"}
    except Exception as e:
...
def run_pipeline(
    ...
    console.print(Panel.fit(
        f"[bold]ScoutPilot Pipeline[/bold] ({mode})",
        border_style="blue",
    ))
