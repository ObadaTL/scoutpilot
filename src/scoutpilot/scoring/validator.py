...
a profile dict (from scoutpilot.config.load_profile()) and validates against the user's
...
# Canonical section name -> accepted header variants. Shared by validation
# (does the tailored doc have these?) and by extra-section extraction (which
# headers in the base CV are NOT one of these, and so should pass through
# verbatim rather than being dropped by the LLM's fixed JSON schema).
...
_NUMBER_TOKEN = re.compile(r"\d[\d,]*(?:\.\d+)?\s*(?:%|\+|x|X)?")


def find_fabricated_numbers(tailored_text: str, original_text: str, real_metrics: list[str] | None = None) -> list[str]:
    """Find number-bearing claims in tailored text with no basis in the original.

    validate_json_fields operates on the raw LLM JSON and never compares
    against the original resume, so an invented "80%" or "500+ devices" that
    reads as plausible prose sails straight through it. This checks the
    ASSEMBLED tailored text against the original resume text (and the
    profile's declared real_metrics) and flags any number token that
    appears in neither -- catching exactly the kind of fabrication that
    matters most (a fake statistic on someone's CV), while leaving dates
    (plausible 4-digit years) alone since those get reused legitimately.

    Args:
        tailored_text: The assembled tailored resume/CV text.
        original_text: The candidate's base resume/CV text.
        real_metrics: Profile-declared real metrics (resume_facts.real_metrics).

    Returns:
        List of unique fabricated number tokens found (e.g. ["80%", "500+"]).
    """
    real_metrics = real_metrics or []
    found: list[str] = []
    seen: set[str] = set()
    for match in _NUMBER_TOKEN.finditer(tailored_text):
        token = match.group().strip()
        digits = re.sub(r"[^\d]", "", token)
        if not digits or token in seen:
            continue
        # Bare 4-digit numbers in a plausible year range are dates, not metrics
        if len(digits) == 4 and token == digits and int(digits) in _YEAR_RANGE:
            continue
        if token in original_text:
            continue
        if any(token in m for m in real_metrics):
            continue
        seen.add(token)
        found.append(token)
    return found
...
def validate_json_fields(data: dict, profile: dict, mode: str = "normal") -> dict:
    ...
    if skills_text:
        ...
        for fake in FABRICATION_WATCHLIST:
            if len(fake) <= 2:
                continue
            if any(fake in s or s in fake for s in allowed_skills):
                continue
    ...
    if isinstance(data["experience"], list):
        for company in preserved_companies:
            comp_clean = company.lower().replace(" ltd", "").replace(" limited", "").strip()
            has_company = any(
                comp_clean in str(e.get("header", "")).lower() or
                comp_clean in str(e.get("subtitle", "")).lower() or
                comp_clean in " ".join(str(b) for b in e.get("bullets", [])).lower()
