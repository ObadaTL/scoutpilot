...
All notable changes to ScoutPilot will be documented in this file.
...
### Added
- **Parallel workers for discovery/enrichment** - `scoutpilot run --workers N` enables
...
- **Apply utility modes** - `--gen` (generate prompt for manual debugging), `--mark-applied`,
  `--mark-failed`, `--reset-failed` flags on `scoutpilot apply`
- **Dry-run mode** - `scoutpilot apply --dry-run` fills forms without clicking Submit
...
- **Config YAML not found after install** - moved `config/` into the package at
  `src/scoutpilot/config/` so YAML files (employers, sites, searches) ship with `pip install`
...
- Interactive setup wizard (`scoutpilot init`)
